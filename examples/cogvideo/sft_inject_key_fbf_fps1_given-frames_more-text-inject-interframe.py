    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            with accelerator.accumulate(models_to_accumulate):
                ...
                # frame diff
                frame_diff = model_pred[:, 1:] - model_pred[:, :-1]
                inter_frame_loss = torch.mean(frame_diff ** 2)
                
                large_frame_loss = model_pred[:, :1] - model_pred[:, -1:]
                large_frame_loss = -1 * torch.mean(large_frame_loss ** 2)
                ...
                loss = loss + (inter_frame_loss + large_frame_loss) * args.interframe_weight
            ...
            logs = {"loss": loss.detach().item(),
                    "inter_loss": inter_frame_loss.detach().item(),
                    "large_frame_loss": large_frame_loss.detach().item(),
