    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            with accelerator.accumulate(transformer):
                ...
                choose_timestep_idx = step % len(timesteps_list)
                timesteps_list = timesteps_list[choose_timestep_idx:]
                
                # DDIM n-1 no_grad denoising
                with torch.no_grad():
                    noisy_coarse_video_latents = scheduler.add_noise(coarse_video_latents, noise, timesteps_list[0])
                    ...
                    for i, t in enumerate(timesteps_list[:-1]):
                        ...
                        noisy_coarse_video_latents, old_pred_original_sample = scheduler.step(
                            ...
                            t,
                            timesteps_list[i - 1] if i > 0 else None,
                ...
                # NOTE last step denoising process, with grad
                for i, t in enumerate(timesteps_list[-1:]):
                    noisy_model_input = torch.cat([noisy_coarse_video_latents, coarse_image_latents], dim=2)
                    model_output = transformer(
                        hidden_states=noisy_model_input,
                ...
                # n-1 predict x_0
                clean_x_0 = noisy_coarse_video_latents
                ...
                with torch.no_grad():
                    ...
                    for b_idx in range(post_continue_video_latents.shape[0]):
                        ...
                        noisy_current_continue_latents = scheduler.add_noise(current_continue_latents, noise, timesteps_list[0])
                ...
                distill_loss = F.mse_loss(clean_x_0.float(), real_output.detach().float(), reduction="mean")
