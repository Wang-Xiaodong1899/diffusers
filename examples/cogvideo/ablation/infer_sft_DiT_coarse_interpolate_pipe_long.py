...
def main(val_s: int=0, val_e: int=10, num_frames: int=11, start_frame: int=0, item_per_scene: int=5):
    ...
    root_root_dir = "/data/wangxd/IJCAI25/Ablation/DiT_coarse/"
    ...
    for i in tqdm(range(val_s, val_e)): # each scene
        ...
        root_dir = os.path.join(root_root_dir, f"{i}")
        ...
        for key_idx in tqdm(key_indexs):
            ...
            gen_dir = os.path.join(root_dir, f"{key_idx:04d}", "generate")
            ...
            # MP42images
            mp4_path = os.path.join(root_dir, f"{key_idx:04d}.mp4")
