...
from diffusers.models.transformers.cogvideox_transformer_3d_interpolate import CogVideoXTransformer3DModel as InterCogVideoXTransformer3DModel
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            with accelerator.accumulate(models_to_accumulate):
                ...
                loss_s = torch.mean((weights * (clean_x_0[:, 0] - target[:, 0]) ** 2)).detach()
                loss_e = torch.mean((weights * (clean_x_0[:, -1] - target[:, -1]) ** 2)).detach()
                
                # print(f'loss: {loss.mean()}, distill_loss: {distill_loss}')
