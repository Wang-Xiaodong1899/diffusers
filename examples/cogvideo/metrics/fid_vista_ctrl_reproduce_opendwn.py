...
def main(
    root_dir= '/data/wuzhirong/exp/IJCAI25/control/vista',
    version = None,
    num_frames=8,
    eval_frames=25,
    batch_size=16, 
    ...
    print(f'eval videos at {root_dir} with {eval_frames} frames')
    ...
    for tgt_dir in tgt_dirs: # scenes
        ...
        for file in tqdm(video_files):
            if file.split('.')[-1] == 'mp4':
                syn_videos_paths.append(os.path.join(root_dir, tgt_dir, file)) # only load eval_frames
    ...
    import json
    with open('/data/wuzhirong/exp/IJCAI25/control/anno.json', 'r') as f:
        anno_data = json.load(f)
    
    scene_names = anno_data.keys()
    
    selected_items = [0, 3, 6, 9, 12, 15, 18, 21, 24, 27]
    
    for scene in tqdm(scene_names):
        for item_id in selected_items:
            item = anno_data[scene][item_id] # first item
            frames_path = item["frames"]
            frames_path = [os.path.join(NUSCENES_ROOT, impath) for impath in frames_path]
            real_frames_paths.extend(frames_path[frame_start:eval_frames])
